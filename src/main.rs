...
use entity::users::USER_PASSWORD_SALT;
...
async fn add_task(
    ...
    match active_task.insert(db).await {
        Ok(result) => result,
        Err(_) => return Flash::error(Redirect::to("/"), "Issue creating the task"),
    };
...
async fn index(
    ...
    Ok(Template::render(
        "todo_list",
        json!({ "tasks": tasks, "flash": flash.map(FlashMessage::into_inner), "number_of_pages": number_of_pages, "current_page": page }),
    ))
}
...
async fn create_account(
    ...
    let hash =
        match argon2::hash_encoded(user.password.as_bytes(), USER_PASSWORD_SALT, &hash_config) {
            Ok(result) => result,
            Err(_) => return Flash::error(Redirect::to("/signup"), "Issue creating account"),
        };
...
async fn verify_account(
    ...
    let stored_user = match Users::find()
        ...
        .await
    {
        Ok(model_or_null) => match model_or_null {
            Some(model) => model,
            None => {
                return login_error();
            }
    ...
    let is_password_correct =
        match argon2::verify_encoded(&stored_user.password, user.password.as_bytes()) {
            Ok(result) => result,
            Err(_) => {
                return Flash::error(
                    Redirect::to("/login"),
                    "Encountered an issue processing your account",
                )
            }
        };
